...
while(1),
    ...
    if len == 0 || len < 0, % if the client does not receive the response
        disp('server not responding...reasking to acknowledge');
        pnet('closeall');
        udp = pnet('udpsocket',3333);
        continue;
...
% value to 0, which is not needed here. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if UdpTcp == 1,
    windows = 0;
    for length = [200,1400]
        for bandwidth = linspace(256*1024,oneM * 4,5)
	%	for bandwidth = oneM*10
            tic;
            ...
            disp(nem);
            toker = toc;
            disp(toker) % Display how much time it took to run a experiment.
