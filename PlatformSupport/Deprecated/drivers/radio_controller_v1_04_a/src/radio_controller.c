...
// This library is made to work with Radio Controller v1.04.a
//
...
unsigned short TX_ANTSW_1 = 0x2;
unsigned short RX_ANTSW_1 = 0x1;
unsigned short TX_ANTSW_2 = 0x2;
unsigned short RX_ANTSW_2 = 0x1;
unsigned short TX_ANTSW_3 = 0x2;
unsigned short RX_ANTSW_3 = 0x1;
unsigned short TX_ANTSW_4 = 0x2;
unsigned short RX_ANTSW_4 = 0x1;
...
void WarpRadio_InitializeRadio(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {						// Make sure that radio 1 is to be used.
		TX_ANTSW_1 = 0x2;						// Put antenna switch configurations in default state.
		RX_ANTSW_1 = 0x1;
		...
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) | 0x00000010)); // RXHP software control on
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFF7C7F)); // Turn off the Sleep for RSSI and DCS goes to 0
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) | (RX_ANTSW_1<<7))); // Put Antenna Sw into Receive position
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {						// Make sure that radio 2 is to be used.
		TX_ANTSW_2 = 0x2;						// Put antenna switch configurations in default state.
		RX_ANTSW_2 = 0x1;
		...
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) | 0x00000010)); // RXHP software control on
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFF7C7F)); // Turn off the Sleep for RSSI and DCS goes to 0
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) | (RX_ANTSW_2<<7))); // Put Antenna Sw into Receive position
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {						// Make sure that radio 3 is to be used.
		TX_ANTSW_3 = 0x2;						// Put antenna switch configurations in default state.
		RX_ANTSW_3 = 0x1;
		...
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) | 0x00000010)); // RXHP software control on
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFF7C7F)); // Turn off the Sleep for RSSI and DCS goes to 0
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) | (RX_ANTSW_3<<7))); // Put Antenna Sw into Receive position
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {						// Make sure that radio 4 is to be used.
		TX_ANTSW_4 = 0x2;						// Put antenna switch configurations in default state.
		RX_ANTSW_4 = 0x1;
		...
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) | 0x00000010)); // RXHP software control on
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFF7C7F)); // Turn off the Sleep for RSSI and DCS goes to 0
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) | (RX_ANTSW_4<<7))); // Put Antenna Sw into Receive position
	}
}
...
void WarpRadio_RxEnable(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFE39)); // Disable Rx and Tx, set both antenna sw to 0
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) | 0x00000008)); // Disable SHDN and amplifiers
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) | (RX_ANTSW_1<<7)));	// Put Antenna Sw in Receive mode
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) | 0x00000004));	// Turn on the recieve enable mode
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFE39)); // Disable Rx and Tx, set both antenna sw to 0
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) | 0x00000008)); // Disable SHDN and amplifiers
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) | (RX_ANTSW_2<<7)));	// Put Antenna Sw in Receive mode
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) | 0x00000004));	// Turn on the recieve enable mode
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFE39)); // Disable Rx and Tx, set both antenna sw to 0
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) | 0x00000008)); // Disable SHDN and amplifiers
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) | (RX_ANTSW_3<<7)));	// Put Antenna Sw in Receive mode
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) | 0x00000004));	// Turn on the recieve enable mode
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFE39)); // Disable Rx and Tx, set both antenna sw to 0
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) | 0x00000008)); // Disable SHDN and amplifiers
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) | (RX_ANTSW_4<<7)));	// Put Antenna Sw in Receive mode
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) | 0x00000004));	// Turn on the recieve enable mode
	}
}
...
void WarpRadio_RxDisable(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFB)); // Turn off receive enable bit
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFB)); // Turn off receive enable bit
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFB)); // Turn off receive enable bit
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFB)); // Turn off receive enable bit
	}
...
void WarpRadio_TxEnable(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFE79)); // Disable Rx, set both antsw to 0
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) | (TX_ANTSW_1<<7))); // Put Antenna Sw to Transmit mode
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) | 0x0000000A)); // Turn on transmit mode
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFE79)); // Disable Rx, set both antsw to 0
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) | (TX_ANTSW_2<<7))); // Put Antenna Sw to Transmit mode
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) | 0x0000000A)); // Turn on transmit mode
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFE79)); // Disable Rx, set both antsw to 0
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) | (TX_ANTSW_3<<7))); // Put Antenna Sw to Transmit mode
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) | 0x0000000A)); // Turn on transmit mode
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFE79)); // Disable Rx, set both antsw to 0
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) | (TX_ANTSW_4<<7))); // Put Antenna Sw to Transmit mode
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) | 0x0000000A)); // Turn on transmit mode
	}
...
void WarpRadio_TxDisable(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFD)); // Turn off transmit mode
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFD)); // Turn off transmit mode
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFD)); // Turn off transmit mode
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFD)); // Turn off transmit mode
	}
...
void WarpRadio_24AmpEnable(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) | 0x00000020)); // Turn on 2.4GHz amp
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) | 0x00000020)); // Turn on 2.4GHz amp
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) | 0x00000020)); // Turn on 2.4GHz amp
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) | 0x00000020)); // Turn on 2.4GHz amp
	}
...
void WarpRadio_24AmpDisable(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFDF)); // Turn off 2.4GHz amp
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFDF)); // Turn off 2.4GHz amp
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFDF)); // Turn off 2.4GHz amp
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFDF)); // Turn off 2.4GHz amp
	}
...
void WarpRadio_5AmpEnable(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) | 0x00000040)); // Turn on 5GHz amp
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) | 0x00000040)); // Turn on 5GHz amp
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) | 0x00000040)); // Turn on 5GHz amp
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) | 0x00000040)); // Turn on 5GHz amp
	}
...
void WarpRadio_5AmpDisable(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFBF)); // Turn off 2.4GHz amp
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFBF)); // Turn off 2.4GHz amp
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFBF)); // Turn off 2.4GHz amp
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFBF)); // Turn off 2.4GHz amp
	}
...
void WarpRadio_SHDNEnable(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF9)); // Turn off recieve and transmit modes
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) | 0x00000001)); // Assert shutdown enable
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF9)); // Turn off recieve and transmit modes
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) | 0x00000001)); // Turn off shutdown enable
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF9)); // Turn off recieve and transmit modes
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) | 0x00000001)); // Turn off shutdown enable
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF9)); // Turn off recieve and transmit modes
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) | 0x00000001)); // Turn off shutdown enable
	}
}
...
void WarpRadio_SHDNDisable(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFE)); //De-assert SHDN
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFE)); //De-assert SHDN
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFE)); //De-assert SHDN
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFE)); //De-assert SHDN
	}
...
void WarpRadio_Reset(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF9)); // Turn off TxEnable and RxEnable
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) | 0x00000001)); // Assert Shutdown
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) | 0x00000006)); // Turn on TxEnable and RxEnable
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF9)); // Turn off TxEnable and RxEnable
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFE)); // De-assert shutdown
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF9)); // Turn off TxEnable and RxEnable
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) | 0x00000001)); // Assert Shutdown
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) | 0x00000006)); // Turn on TxEnable and RxEnable
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF9)); // Turn off TxEnable and RxEnable
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFE)); // De-assert shutdown
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF9)); // Turn off TxEnable and RxEnable
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) | 0x00000001)); // Assert Shutdown
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) | 0x00000006)); // Turn on TxEnable and RxEnable
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF9)); // Turn off TxEnable and RxEnable
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFE)); // De-assert shutdown
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF9)); // Turn off TxEnable and RxEnable
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) | 0x00000001)); // Assert Shutdown
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) | 0x00000006)); // Turn on TxEnable and RxEnable
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF9)); // Turn off TxEnable and RxEnable
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFFE)); // De-assert shutdown
	}
...
void WarpRadio_RxHp_Soft_Control_Enable(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) | 0x00000010)); // 162 Turn on transmit mode, and antsw[0]
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) | 0x00000010)); // Turn on transmit mode, and antsw[0]
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) | 0x00000010)); // Turn on transmit mode, and antsw[0]
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) | 0x00000010)); // Turn on transmit mode, and antsw[0]
	}
...
void WarpRadio_RxHp_Soft_Control_Disable(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFEF)); // 162 Turn on transmit mode, and antsw[0]
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFEF)); // Turn on transmit mode, and antsw[0]
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFEF)); // Turn on transmit mode, and antsw[0]
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFEF)); // Turn on transmit mode, and antsw[0]
	}
...
void WarpRadio_RxHpEnable(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) | 0x00000008));
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) | 0x00000008));
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) | 0x00000008));
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) | 0x00000008));
	}
...
void WarpRadio_RxHpDisable(unsigned int* baseaddr, unsigned char radios) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF7));
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF7));
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF7));
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) & 0xFFFFFFF7));
	}
...
char WarpRadio_AntennaConfig(unsigned int* baseaddr, char mode, unsigned char radios) {

	RADIO_CONTROLLER_mWriteSlaveReg6((volatile)baseaddr, (SLAVEMASK & radios));			// Select Radios affected by this function

	if (mode == 1 || mode == 2 || mode == 3 || mode == 4) {
		char bit0 = mode & 0x01;
		char nbit0 = ~mode & 0x01;
		char bit1 = (mode & 0x02)>>1;
		char nbit1 = (~mode & 0x02)>>1;

		if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
			TX_ANTSW_1 = (bit0<<1) | nbit0;
			RX_ANTSW_1 = (bit1<<1) | nbit1;
		}
		if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
			TX_ANTSW_2 = (bit0<<1) | nbit0;
			RX_ANTSW_2 = (bit1<<1) | nbit1;
		}
		if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
			TX_ANTSW_3 = (bit0<<1) | nbit0;
			RX_ANTSW_3 = (bit1<<1) | nbit1;
		}
		if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
			TX_ANTSW_4 = (bit0<<1) | nbit0;
			RX_ANTSW_4 = (bit1<<1) | nbit1;
		}
		return WARP_SUCCESS;
	}
	else {
...
void WarpDac_InitializeDAC(unsigned int* baseaddr, unsigned char dacs) {
	...
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & DAC1MASK) > 0) {		// Store local copies for the DACs that are affected by this function call
		...
		RADIO_CONTROLLER_mWriteSlaveReg0((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg0((volatile)baseaddr) & SLVREGRDMSK) & 0xFFDFFFFF));	// Set TX_DAC_RESET to 0
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & DAC2MASK) > 0) {
		REG_DAC2_REGISTER_0 = (char)0x04;
		RADIO_CONTROLLER_mWriteSlaveReg1((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg1((volatile)baseaddr) & SLVREGRDMSK) & 0xFFDFFFFF));	// Set TX_DAC_RESET to 0
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & DAC3MASK) > 0) {
		REG_DAC3_REGISTER_0 = (char)0x04;
		RADIO_CONTROLLER_mWriteSlaveReg2((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg2((volatile)baseaddr) & SLVREGRDMSK) & 0xFFDFFFFF));	// Set TX_DAC_RESET to 0
	}
	if((RADIO_CONTROLLER_mReadSlaveReg6((volatile)baseaddr) & DAC4MASK) > 0) {
		REG_DAC4_REGISTER_0 = (char)0x04;
		RADIO_CONTROLLER_mWriteSlaveReg3((volatile)baseaddr, ((RADIO_CONTROLLER_mReadSlaveReg3((volatile)baseaddr) & SLVREGRDMSK) & 0xFFDFFFFF));	// Set TX_DAC_RESET to 0
	}
