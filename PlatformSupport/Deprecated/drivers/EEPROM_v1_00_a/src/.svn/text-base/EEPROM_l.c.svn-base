...
void EEPROM_EnableInterrupt(void * baseaddr_p)
{
   ...
   */
  EEPROM_mWriteReg(baseaddr, EEPROM_INTR_IER_OFFSET, 0x00000001);

  /*
   * Enable all possible interrupt sources from device.
   */
  EEPROM_mWriteReg(baseaddr, EEPROM_INTR_DIER_OFFSET,
    INTR_TERR_MASK
    | INTR_DPTO_MASK
    | INTR_IPIR_MASK
    );
   ...
   */
  EEPROM_mWriteReg(baseaddr, EEPROM_INTR_DGIER_OFFSET, INTR_GIE_MASK);
}

/**
 *
 * Example interrupt controller handler for EEPROM device.
...
void EEPROM_Intr_DefaultHandler(void * baseaddr_p)
{
   ...
   */
  IntrStatus = EEPROM_mReadReg(baseaddr, EEPROM_INTR_ISR_OFFSET);
   ...
   */
  EEPROM_mWriteReg(baseaddr, EEPROM_INTR_ISR_OFFSET, IntrStatus);
}
...
// Sets the clock divisor based on the opb_clock speed
// baseaddr is the base address of the EEPROM device.
// OPB_CLOCK defined in EEPROM_l.h determines which clock division should be made
//  OPB_CLOCK     Mhz (range)
//      1:        4.0   <5.0
//      2:        5.0   <6.0
//      3:        6.0   <7.0    
...
//     19:       96.0 <112.0
//     20:      112.0 <128.0
//char WarpEEPROM_Initialize(unsigned int* baseaddr, char EEPROM_select)
char WarpEEPROM_Initialize(unsigned int* baseaddr)
{
   ...
   EEPROM_mReset((int)baseaddr); // Reset EEPROM Master
