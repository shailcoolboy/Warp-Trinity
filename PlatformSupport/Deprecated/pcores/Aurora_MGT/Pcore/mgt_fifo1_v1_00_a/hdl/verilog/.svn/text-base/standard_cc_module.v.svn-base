 ...
 //________________________Clock Correction State Machine__________________________
    ...
    initial
     ...
     // is used to prevent UFC messages from starting.
      ...
      // For 1 lane, we use a 4-cycle count.
      always @(posedge USER_CLK)
          prepare_count_r <=  `DLY    {4'd0,cc_idle_count_done_c,prepare_count_r[4:6]};
    ...
    always @(posedge USER_CLK)
         ...
         else if(prepare_count_r[7])                WARN_CC    <=  `DLY    1'b0;
    ...
    assign start_cc_c   =   prepare_count_r[7] || (CHANNEL_UP && reset_r);
     ...
     always @(posedge USER_CLK)
         cc_count_r      <=  `DLY    {3'b000,(!CHANNEL_UP|prepare_count_r[7]),cc_count_r[3:4]};
