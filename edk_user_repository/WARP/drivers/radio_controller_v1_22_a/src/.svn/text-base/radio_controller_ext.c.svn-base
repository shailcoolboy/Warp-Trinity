...
void WarpRadio_v1_SetTxTiming(unsigned int radios, unsigned char dly_TxEn, unsigned short dly_TxStart, unsigned char dly_GainRampEn, unsigned char dly_PowerAmpEn) {
	...
	//TxTiming values stored in two registers per radio:
	// GainRampEn -	slv_reg{13,14,15,16}[0:7]
	// PowerAmpEn -	slv_reg{13,14,15,16}[8:15]
	// TxEn -		slv_reg{13,14,15,16}[16:23]
	// TxStart -	slv_reg{09,10,11,12}[16:27]
	//Endian change swaps LSB/MSB in C<->Verilog

	unsigned int regValA, regValB;
	unsigned int regMaskA = 0xFFFFFF00; //mask[0:23] for regs13/14/15/16
	unsigned int regMaskB = 0x0000FFF0; //mask[16:27] for regs09/10/11/12

	//Mask the input parameters to their max sizes, then shift to their positions in each register
	regValA =	((dly_GainRampEn & 0xFF) << 24 ) | ((dly_PowerAmpEn & 0xFF) << 16 ) | ((dly_TxEn & 0xFF) << 8 );
	regValB = 	((dly_TxStart & 0xFFF) << 4);

	if(((SLAVEMASK & radios) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg13((volatile)radio_controller_baseaddr, (RADIO_CONTROLLER_mReadSlaveReg13((volatile)radio_controller_baseaddr) & ~regMaskA) | regValA );
		RADIO_CONTROLLER_mWriteSlaveReg9((volatile)radio_controller_baseaddr, (RADIO_CONTROLLER_mReadSlaveReg9((volatile)radio_controller_baseaddr) & ~regMaskB) | regValB );
	}
	if(((SLAVEMASK & radios) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg14((volatile)radio_controller_baseaddr, (RADIO_CONTROLLER_mReadSlaveReg14((volatile)radio_controller_baseaddr) & ~regMaskA) | regValA );
		RADIO_CONTROLLER_mWriteSlaveReg10((volatile)radio_controller_baseaddr, (RADIO_CONTROLLER_mReadSlaveReg10((volatile)radio_controller_baseaddr) & ~regMaskB) | regValB );
	}
	if(((SLAVEMASK & radios) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg15((volatile)radio_controller_baseaddr, (RADIO_CONTROLLER_mReadSlaveReg15((volatile)radio_controller_baseaddr) & ~regMaskA) | regValA );
		RADIO_CONTROLLER_mWriteSlaveReg11((volatile)radio_controller_baseaddr, (RADIO_CONTROLLER_mReadSlaveReg11((volatile)radio_controller_baseaddr) & ~regMaskB) | regValB );
	}
	if(((SLAVEMASK & radios) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg16((volatile)radio_controller_baseaddr, (RADIO_CONTROLLER_mReadSlaveReg16((volatile)radio_controller_baseaddr) & ~regMaskA) | regValA );
		RADIO_CONTROLLER_mWriteSlaveReg12((volatile)radio_controller_baseaddr, (RADIO_CONTROLLER_mReadSlaveReg12((volatile)radio_controller_baseaddr) & ~regMaskB) | regValB );
	}
...
void WarpRadio_v1_SetTxGainTiming(unsigned int radios, unsigned char TxGainTarget, unsigned char TxGainStep, unsigned char TxGainTimeStep) {
	...
	//TxTiming values stored in two registers per radio:
	// TxGainTarget -	slv_reg{09,10,11,12}[0:5]
	// TxGainStep -		slv_reg{09,10,11,12}[6:9]
	// TxGainTimeStep -	slv_reg{09,10,11,12}[10:13]
	//Endian change swaps LSB/MSB in C<->Verilog

	unsigned int regValA;
	unsigned int regMaskA = 0xFFFC0000; //mask[0:13] for regs9/10/11/12

	regValA = ((TxGainTarget & 0x3F) << 26) | ((TxGainStep & 0xF) << 22) | ((TxGainTimeStep & 0xF) << 18);
	
	if(((SLAVEMASK & radios) & RAD1MASK) > 0) {		// Check to make sure if Radio 1 is being used
		RADIO_CONTROLLER_mWriteSlaveReg9((volatile)radio_controller_baseaddr, (RADIO_CONTROLLER_mReadSlaveReg9((volatile)radio_controller_baseaddr) & ~regMaskA) | regValA );
	}
	if(((SLAVEMASK & radios) & RAD2MASK) > 0) {		// Check to make sure if Radio 2 is being used
		RADIO_CONTROLLER_mWriteSlaveReg10((volatile)radio_controller_baseaddr, (RADIO_CONTROLLER_mReadSlaveReg10((volatile)radio_controller_baseaddr) & ~regMaskA) | regValA );
	}
	if(((SLAVEMASK & radios) & RAD3MASK) > 0) {		// Check to make sure if Radio 3 is being used
		RADIO_CONTROLLER_mWriteSlaveReg11((volatile)radio_controller_baseaddr, (RADIO_CONTROLLER_mReadSlaveReg11((volatile)radio_controller_baseaddr) & ~regMaskA) | regValA );
	}
	if(((SLAVEMASK & radios) & RAD4MASK) > 0) {		// Check to make sure if Radio 4 is being used
		RADIO_CONTROLLER_mWriteSlaveReg12((volatile)radio_controller_baseaddr, (RADIO_CONTROLLER_mReadSlaveReg12((volatile)radio_controller_baseaddr) & ~regMaskA) | regValA );
	}
