...
pfile = 'channelTrace0125_indoor_phase_sync.txt';
afile = 'channelTrace0125_indoor_amplitude_sync.txt';
getTrace = 1; % 1: collect traces
procSamples = 1; % process traces


CarrierChannel = 14; % Channel in the 2.4 GHz band. In [1:14]
TxGain_RF = 30; %40;  % Tx RF Gain. In [0:63]
TxGain_BB = 1;%1;% Tx Baseband Gain. In [0:3]

RxGain_BB = 20;% 13;% Rx Baseband Gain. In [0:31]
...
%[socketHandles, packetNum] = warplab_initialize(numRxNode+1); %test
...
for rxn = 1:numRxNode
for rxa = 1:numRxAntenna
    sf = sprintf('singleToneSamples_a%d_r%d_t%d', rxa, rxn, txID);
...
for txa = 1:numTxAntenna
    ...
    for rxa = 1:numRxAntenna
		...
		if (rxa == 1)
	    	...
	    	%testrel = channel_phase(:,rxa,rxn,txa,txn)-channel_phase(:,1,rxn,ta,txn);
			rel = channel_phase(:,rxa,rxn,txa,txn);
		end
		if (mean(rel(2000:end) < 0)) rel = rel + 360; end
...
subplot(2,1,2);
sqamp = squeeze(channel_amplitude(:,1,1,2,1));
samp1 =sqamp(sT:end);
plot(samp1);
mamp = mean(samp1);
vamp = std(samp1);
...
fprintf(1, 'Amp ant1 mean=%f std/mean=%f\n', mamp, vamp/mamp);
sqamp = squeeze(channel_amplitude(:,1,1,2,1));
samp1 =sqamp(sT:end);
mamp = mean(samp1);
vamp = std(samp1);
fprintf(1, 'Amp ant2 mean=%f std/mean=%f\n', mamp, vamp/mamp);
...
title(sout);
subplot(3,1,3);
pdiff = squeeze(rel_phase(:,1,1,1,1)) - squeeze(rel_phase(:,1,1,2,1));
if (mean(pdiff(1000:end) < 0)) pdiff = pdiff + 360; end
pdiff = pdiff(sT:end);
plot(pdiff);
xlabel('n (samples)'); ylabel('Degrees');
