...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% In this lab exercise you will write a matlab script that generates a 
% bitstream, modulates the bitstream using DQPSK, transmits the modulated 
% symbols over a wireless channel using Warplab, and demodulates the 
% received signal to obtain the transmitted bits. Bit error rate (BER) is 
% computed by comparing the transmitted bitstream with the bitstream 
...
% You will write a matlab script that implements the seven steps above. 
...
%           transmit is the 'ytx_mod_filt' vector. Set TxLength equal to 
%           the length of the 'ytx_mod_filt' vector. You can use Matlab's 
%           'length' function   
...
% Hint 1: The second argument of the 'warplab_writeSMWO' function is the
% vector to be downloaded and it must be a row vector. Notice that 
% 'ytx_mod_filt' is a column vector. To make 'ytx_mod_filt' a row vector 
% simply take transpose (only transpose, NOT conjugate transpose). The 
% transpose can be obtained using Matlab's 'transpose' function

% Hint 2: The third argument of 'warplab_writeSMWO' is RADIO2_TXDATA,
% RADIO2_TXDATA is defined in 'warplab_siso_defines'. RADIO2_TXDATA can be 
...
% Hint 1: For this exercise, the third argument of the 'warplab_readSMRO' 
% function is equal to 'TxLength'(in this exercise CaptOffset is equal to
% zero)

% Hint 2: The second argument of 'warplab_readSMRO' is RADIO2_RXDATA,
% RADIO2_RXDATA is defined in 'warplab_siso_defines'. RADIO2_RXDATA can be 
% understood as an id that identifies the receiver buffer.   

% Read back the received samples
[RawRxData] = warplab_readSMRO(udp_RxA, RADIO2_RXDATA, TxLength);
...
figure;
subplot(2,2,1);
plot(real(ytx_mod_filt));
title('Tx I');
xlabel('n (samples)'); ylabel('Amplitude');
axis([0 2^14 -1 1]); % Set axis ranges.
subplot(2,2,2);
plot(imag(ytx_mod_filt));
title('Tx Q');
xlabel('n (samples)'); ylabel('Amplitude');
axis([0 2^14 -1 1]); % Set axis ranges.
subplot(2,2,3);
...
% goes through 2 filtering stages (one at the Tx and the other one at the Rx)
