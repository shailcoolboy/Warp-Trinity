		...
		if (bytesReceived > 0) {	// Check for valid packet
			...
			switch(command) {			// Decode on incoming command
				...
				case INITIALIZE:
					...
					warplab_mimo_2x2_WriteReg_DoneReading(XPAR_WARPLAB_MIMO_2X2_OPBW_0_BASEADDR, 1); // Cycle Done reading signal
				...
				case CAPT_OFFSET:
					...
					if(0 == ReadWrite)
					{
						...
						sendPtr32[3] = warplab_mimo_2x2_ReadReg_CaptOffset(XPAR_WARPLAB_MIMO_2X2_OPBW_0_BASEADDR);

						xilsock_sendto(sock, sendBuffer, 16, (struct sockaddr *)&addr, alen);
					}
					if(1 == ReadWrite)
					{
						warplab_mimo_2x2_WriteReg_CaptOffset(XPAR_WARPLAB_MIMO_2X2_OPBW_0_BASEADDR, (unsigned int)receivePtr32[3]);
						sendACK(packetNo, command);
					}
				break;
