...
% CaptOffset: Value of the Capture offset. In continuous transmission mode
%             the receiver captures CaptOffset samples of noise and the
%             first 2^14-CaptOffset samples of data.
...
% Download the CaptOffset parameter to node 2 using the 
...
% the code). Since CaptOffset is only required at the receiver 
...
% to node 2 is 'udp_node2'.

% 2. The second argument of the 'warplab_writeRegister' function identifies 
% the parameter that will be downloaded. The id for the CaptOffset
% parameter is 'CAPT_OFFSET'. 
...
% Download the TxLength and TxMode parameters to node 1 using the 
% 'warplab_writeRegister' function.
...
% TxMode are only required at the transmitter download the TxLength and
...
% 2. The second argument of the 'warplab_writeRegister' function identifies 
% the parameter that will be downloaded. The id for the TxLength
% parameter is 'TX_LENGTH' and the id for the TxMode parameter is
% 'TX_MODE'.

% 3. The third argument of the 'warplab_writeRegister' function is the
% value the parameter must be set to. The values to download have been
% stored in the 'TxLength' and 'TxMode' variables.

% 4. The 'warplab_writeRegister' function has been used in previous
% exercises.

% 5. Call the 'warplab_writeRegister' twice. One time to download the
% TxLength and one time to download the TxMode. The 'warplab_writeRegister'
% function can only set one register (parameter) per node at a time
