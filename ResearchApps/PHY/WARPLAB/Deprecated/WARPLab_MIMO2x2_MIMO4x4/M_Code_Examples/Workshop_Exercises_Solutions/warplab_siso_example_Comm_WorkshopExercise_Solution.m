...
%             equal to 100. This value of CaptOffset will allow detection
...
CaptOffset = 100; % Number of noise samples per Rx capture. In [0:2^14]
TxLength =length(ytx_mod_filt_up); % Length of transmission. In [0:2^14-1-CaptOffset]
...
% the warplab_mimo_2x2_v03.bit bitstream has 2 transmit buffers and a node 
% programmed with the warplab_mimo_4x4_v03.bit bitstream has 4 transmit 
...
% 1. The first argument of the 'warplab_readSMWO' function identifies the
...
% 2. The second argument of the 'warplab_readSMWO' function identifies the
% receive buffer from which samples will be read. A node programmed with
% the warplab_mimo_2x2_v03.bit bitstream has 2 receive buffers and a node 
% programmed with the warplab_mimo_4x4_v03.bit bitstream has 4 receive 
...
% 4. The 'warplab_readSMWO' function was used in the previous exercise.

% Read back the received samples (Read extra 100 to account for jitter in sync trigger)
[Node2_Radio2_RawRxData] = warplab_readSMRO(udp_node2, RADIO2_RXDATA, TxLength+CaptOffset+100);
%-------------------------------------------------------------------------%
