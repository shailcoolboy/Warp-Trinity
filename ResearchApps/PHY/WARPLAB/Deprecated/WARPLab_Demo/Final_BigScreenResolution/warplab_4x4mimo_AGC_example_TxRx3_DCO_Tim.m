% Simple test from Tx 3 to Rx 3. Quick and dirty version, comments may be
% wrong.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%TxData_2 = 0*exp(t*j*2*pi*1.25e6); %Signal must be a row vector. The signal can
...
warplab_sendCmd(udp_Tx, [RADIO3TXBUFF_TXEN], packetNum);
...
warplab_sendCmd(udp_RxA, [RADIO3RXBUFF_RXEN], packetNum);
...
warplab_sendCmd(udp_Tx, [RADIO3TXBUFF_TXDIS], packetNum);

% Disable the transmitter radio 2 and radio 3
warplab_sendCmd(udp_Tx, [RADIO3_TXDIS], packetNum);

%Enable Radio's receive buffer for storage of samples
warplab_sendCmd(udp_RxA, [RADIO3RXBUFF_RXDIS], packetNum);
