% Simple test from Tx 4 to Rx 4. Quick and dirty version, comments may be
% wrong.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%TxData_1 = 0*exp(t*j*2*pi*1.25e6); %Signal must be a row vector. The signal can
...
% transmitter node receives the sSYNC packet.
...
warplab_sendCmd(udp_RxA, [RADIO4_RXDIS], packetNum);

% Disable transmission of samples stored in radio2 Tx buffer
warplab_sendCmd(udp_Tx, [RADIO4TXBUFF_TXDIS], packetNum);

% Disable the transmitter radio 2 and radio 3
warplab_sendCmd(udp_Tx, [RADIO4_TXDIS], packetNum);

%Enable Radio's receive buffer for storage of samples
warplab_sendCmd(udp_RxA, [RADIO4RXBUFF_RXDIS], packetNum);
