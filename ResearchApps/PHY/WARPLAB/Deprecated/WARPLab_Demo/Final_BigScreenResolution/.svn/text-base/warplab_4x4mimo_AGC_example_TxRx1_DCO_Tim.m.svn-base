% Simple test from Tx 1 to Rx 1. Quick and dirty version, comments may be wrong. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Node 1 will be sept as the transmitter so download Tx gains to node 1.
...
TxData_1 = exp(t*j*2*pi*1.25e6); %Signal must be a row vector. The signal can
...
%TxData_4 = 0*exp(t*j*2*pi*1.25e6); 
...
[RawRxData_1_wDCO] = warplab_readSMRO(udp_RxA, RADIO1_RXDATA, TxLength+CaptOffset);
% Read back the received samples from radio 3
%[RawRxData_4_wDCO] = warplab_readSMRO(udp_RxA, RADIO4_RXDATA, TxLength+CaptOffset);
...
warplab_sendCmd(udp_RxA, [RADIO1_RXDIS], packetNum);

% Disable transmission of samples stored in radio2 Tx buffer
warplab_sendCmd(udp_Tx, [RADIO1TXBUFF_TXDIS], packetNum);

% Disable the transmitter radio 2 and radio 3
warplab_sendCmd(udp_Tx, [RADIO1_TXDIS], packetNum);

%Enable Radio's receive buffer for storage of samples
warplab_sendCmd(udp_RxA, [RADIO1RXBUFF_RXDIS], packetNum);
