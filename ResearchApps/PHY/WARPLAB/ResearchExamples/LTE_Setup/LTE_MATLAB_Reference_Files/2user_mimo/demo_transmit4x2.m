...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This LTE uplink transmission function sends 2 user data block_in1,block_in2 
% into the air with their specific trasmission gains TX_gain1 and TX_gain2, 
% and return the received the signal block_out1, block_out2. The received
% data is already equalized with Zero-forcing equalizer before returned.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [block_out1, block_out2] = demo_transmit4x2(block_in1,block_in2,TX_gain1,TX_gain2)


%% Simulation parameters
dft_size = 72; %Length of DFT
idft_size = 128; %Length of DFT
nsym_CP = 160-128; %cyclic prefix length
filteron = 0; %Low pass filter
...
scale_ShortSyms = 1/max([ max(abs(preamble_I_up2)), max(abs(preamble_Q_up2)) ]);
[preamble_I_up2] = scale_ShortSyms*preamble_I_up2;
[preamble_Q_up2] = scale_ShortSyms*preamble_Q_up2;
...
pilot_tone_1 = ifft([pilot_tone_freq,zeros(1,L)]);
pilot_tone_2 = ifft([zeros(1,L),pilot_tone_freq]);
pilot_CP_1 = pilot_tone_1(end-nsym_CP+1:end);
pilot_CP_2 = pilot_tone_2(end-nsym_CP+1:end);
pilot_tone_1 = [pilot_CP_1 pilot_tone_1];
pilot_tone_2 = [pilot_CP_2 pilot_tone_2];

training.tx1 = pilot_tone_1;
training.tx2 = pilot_tone_1;
training.tx3 = pilot_tone_2;
training.tx4 = pilot_tone_2;
...
Node1_Radio1_TxGain_BB = BB_gain;  % Tx Baseband Gain. In [0:3]
Node1_Radio1_TxGain_RF = TX_gain1;  % Tx RF Gain. In [0:63]
Node1_Radio2_TxGain_BB = BB_gain;  % Tx Baseband Gain. In [0:3]
Node1_Radio2_TxGain_RF = TX_gain1;  % Tx RF Gain. In [0:63]
Node1_Radio3_TxGain_BB = BB_gain;  % Tx Baseband Gain. In [0:3]
Node1_Radio3_TxGain_RF = TX_gain2;  % Tx RF Gain. In [0:63]
Node1_Radio4_TxGain_BB = BB_gain;  % Tx Baseband Gain. In [0:3]
Node1_Radio4_TxGain_RF = TX_gain2;  % Tx RF Gain. In [0:63]
...
block_size = length(block_in1);
block_in1 = block_in1';
block_in2 = block_in2';
stream1 = block_in1(1:block_size/2);
stream2 = block_in1(block_size/2+1:end);
stream3 = block_in2(1:block_size/2);
stream4 = block_in2(block_size/2+1:end);
...
tx3_xsym = bi2de(reshape(stream3,k,length(stream1)/k).','left-msb');
tx4_xsym = bi2de(reshape(stream4,k,length(stream2)/k).','left-msb');
...
tx3_mod = modulate(h_mod, tx3_xsym);
tx3_mod = tx3_mod/(sqrt(M)-1);
tx4_mod = modulate(h_mod, tx4_xsym);
tx4_mod = tx4_mod/(sqrt(M)-1);

%Take the fft
tx1_mod_freq = [fft(tx1_mod(1:dft_size/2));fft(tx1_mod(dft_size/2+1:end))];
tx2_mod_freq = [fft(tx2_mod(1:dft_size/2));fft(tx2_mod(dft_size/2+1:end))];
tx3_mod_freq = [fft(tx3_mod(1:dft_size/2));fft(tx3_mod(dft_size/2+1:end))];
tx4_mod_freq = [fft(tx4_mod(1:dft_size/2));fft(tx4_mod(dft_size/2+1:end))];

%Pad with 0 to ifft size
padding_size = 0.5*(idft_size-dft_size);
padding = zeros(padding_size,1);

tx1_mod_freq = [padding; tx1_mod_freq(1:dft_size/2);zeros(dft_size/2,1); padding; padding; tx1_mod_freq(dft_size/2+1:end); zeros(dft_size/2,1);padding  ];
tx2_mod_freq = [padding; tx2_mod_freq(1:dft_size/2);zeros(dft_size/2,1); padding; padding; tx2_mod_freq(dft_size/2+1:end); zeros(dft_size/2,1);padding  ];
tx3_mod_freq = [padding; zeros(dft_size/2,1);tx3_mod_freq(1:dft_size/2); padding; padding; zeros(dft_size/2,1);tx3_mod_freq(dft_size/2+1:end); padding  ];
tx4_mod_freq = [padding; zeros(dft_size/2,1);tx4_mod_freq(1:dft_size/2); padding; padding; zeros(dft_size/2,1);tx4_mod_freq(dft_size/2+1:end); padding  ];
...
tx3_mod_time = [ifft(tx3_mod_freq(1:idft_size));ifft(tx3_mod_freq(idft_size+1:end))];
tx4_mod_time = [ifft(tx4_mod_freq(1:idft_size));ifft(tx4_mod_freq(idft_size+1:end))];
...
tx3_mod_time = [tx3_mod_time(idft_size-nsym_CP+1:idft_size); tx3_mod_time(1:idft_size);tx3_mod_time(end-nsym_CP+1:end); tx3_mod_time(idft_size+1:end)];
tx4_mod_time = [tx4_mod_time(idft_size-nsym_CP+1:idft_size); tx4_mod_time(1:idft_size);tx4_mod_time(end-nsym_CP+1:end); tx4_mod_time(idft_size+1:end)];
...
ytx3_mod = [training.tx3.'; zeros(length(training.tx3),1); tx3_mod_time ];
ytx4_mod = [zeros(length(training.tx4),1); training.tx4.'; tx4_mod_time ];
...
ytx3_mod_filt = [ShortTrainingSyms_up2_norm.';preamble_upsamp.'; zeros(1,sym_guard*nsamp).'; rcosflt(ytx3_mod,1,nsamp,'filter',rrcfilter)];
ytx4_mod_filt = [ShortTrainingSyms_up2_norm.';preamble_upsamp.'; zeros(1,sym_guard*nsamp).'; rcosflt(ytx4_mod,1,nsamp,'filter',rrcfilter)];
...
ytx1_mod_filt_up = ytx1_mod_filt .* exp(sqrt(-1)*2*pi*5e6*time).';   % why 5e6
ytx2_mod_filt_up = ytx2_mod_filt .* exp(sqrt(-1)*2*pi*5e6*time).';
ytx3_mod_filt_up = ytx3_mod_filt .* exp(sqrt(-1)*2*pi*5e6*time).';   % why 5e6
ytx4_mod_filt_up = ytx4_mod_filt .* exp(sqrt(-1)*2*pi*5e6*time).';

Node1_Radio1_TxData = ytx1_mod_filt_up.';
Node1_Radio2_TxData = ytx2_mod_filt_up.';
Node1_Radio3_TxData = ytx3_mod_filt_up.';
Node1_Radio4_TxData = ytx4_mod_filt_up.';
...
% shaping filter), detect preamble, and downsample output of Matched
% Filter. Frequency domain equlization and convert back to time domain.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if (~ValidStart_1&& ~ValidStart_2)
    ...
    %first_sample_index = 1+2*delay*nsamp;
    ...
    chan.h11 = chan_est(rx1(1:length(training.tx1)),[pilot_tone_freq,pilot_tone_freq], nsym_CP);
    chan.h21 = chan_est(rx1(length(training.tx1)+1:2*length(training.tx1)),[pilot_tone_freq,pilot_tone_freq], nsym_CP);

    chan.h12 = chan_est(rx2(1:length(training.tx1)),[pilot_tone_freq,pilot_tone_freq], nsym_CP);
    chan.h22 = chan_est(rx2(length(training.tx1)+1:2*length(training.tx1)),[pilot_tone_freq,pilot_tone_freq], nsym_CP);
    ...
    %Grab data frames  %not generalized to n frames, make the vector larger
    ...
    %Frequency domain zero forcing equalization. equalize symbol by symbol
    x = [];
    ...
    %use IFFT to get the time domain symbol
    xsym1 = [ifft(x(1,1:dft_size/2)),ifft(x(1,1+dft_size:dft_size+dft_size/2))]*(sqrt(M)-1);
    xsym2 = [ifft(x(2,1:dft_size/2)),ifft(x(2,1+dft_size:dft_size+dft_size/2))]*(sqrt(M)-1);
    xsym3 = [ifft(x(1,1+dft_size/2:dft_size)),ifft(x(1,1+dft_size+dft_size/2:end))]*(sqrt(M)-1);
    xsym4 = [ifft(x(2,1+dft_size/2:dft_size)),ifft(x(2,1+dft_size+dft_size/2:end))]*(sqrt(M)-1);
    
    %% Demodulate
    z1 = demodulate(h_demod, xsym1(1,:));
    z2 = demodulate(h_demod, xsym2(1,:));
    z3 = demodulate(h_demod, xsym3(1,:));
    z4 = demodulate(h_demod, xsym4(1,:));
    ...
    z3_ZF = de2bi(	z3,'left-msb',4); % Convert integers to bits.
    z3_ZF = reshape(z3_ZF.',prod(size(z3_ZF)),1);

    z4_ZF = de2bi(	z4,'left-msb',4); % Convert integers to bits.
    z4_ZF = reshape(z4_ZF.',prod(size(z4_ZF)),1);

end

block_out1 = [z1_ZF' z2_ZF'];
block_out2 = [z3_ZF' z4_ZF'];
