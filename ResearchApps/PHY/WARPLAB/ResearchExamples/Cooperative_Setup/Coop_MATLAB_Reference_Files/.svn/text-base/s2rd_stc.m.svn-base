function [r, d, config, sat_flag, agc_read] = s2rd_STC(config, rrr_cnt)
...
warplab_writeRegister(udp_node1,TX_MODE,TxMode);
	...
	% AGC target parameters. Might need to be changed emperically
	Node2_TargetdBmAGC = -17;     % AGC's target dBm
	Node2_NoiseEstdBmAGC = -95;  % AGC's Noise Estimate in dBm
	Node3_TargetdBmAGC = -15;     % AGC's target dBm
...
% 	% Create a random binary data stream as a column vector.
    ...
    tx1_xsym = floor(M*rand(1,nbits/k/2));
    tx2_xsym = floor(M*rand(1,nbits/k/2));

    tx1_mod = kron(modulate(h_mod, tx1_xsym), [1 0]);
    tx2_mod = kron(modulate(h_mod, tx2_xsym), [1 0]);

    tx1_mod(2:2:end) = -conj(tx2_mod(1:2:end));
    tx2_mod(2:2:end) = conj(tx1_mod(1:2:end));
    
    tx1_xsym_tmp = demodulate(h_demod,tx1_mod);
    tx2_xsym_tmp = demodulate(h_demod,tx2_mod);
    stream1_tmp = de2bi(tx1_xsym_tmp,'left-msb');
    stream1 = reshape(stream1_tmp.',prod(size(stream1_tmp)),1);
    stream2_tmp = de2bi(tx2_xsym_tmp,'left-msb');
    stream2 = reshape(stream2_tmp.',prod(size(stream2_tmp)),1);
    ...
    ytx1_filt = rcosflt(ytx1_mod,1,nsamp,'filter',rrcfilter);
    ytx2_filt = rcosflt(ytx2_mod,1,nsamp,'filter',rrcfilter);
    ytx1_filt = ytx1_filt./max(max([abs(real(ytx1_filt)),abs(imag(ytx1_filt))]))/sqrt(M_T)/1.5;
    ytx2_filt = ytx2_filt./max(max([abs(real(ytx2_filt)),abs(imag(ytx2_filt))]))/sqrt(M_T)/1.5;

    ytx1_mod_filt = [ShortTrainingSyms_up2_norm.'; LongTrainingSyms_up2_norm.'; zeros(32,1); ytx1_filt];
	ytx2_mod_filt = [ShortTrainingSyms_up2_norm.'; LongTrainingSyms_up2_norm.'; zeros(32,1); ytx2_filt];
    ...
    sat_thresh = 0.6;
    sat_flag.r = sum(abs(real(Node2_Radio2_RxData_wDCO(nsym_header*nsamp:end)))>sat_thresh) + ...
        sum(abs(imag(Node2_Radio2_RxData_wDCO(nsym_header*nsamp:end)))>sat_thresh) + ...
        sum(abs(real(Node2_Radio3_RxData_wDCO(nsym_header*nsamp:end)))>sat_thresh) + ...
        sum(abs(imag(Node2_Radio3_RxData_wDCO(nsym_header*nsamp:end)))>sat_thresh);
    sat_flag.d = sum(abs(real(Node3_Radio2_RxData_wDCO(nsym_header*nsamp:end)))>sat_thresh) + ...
        sum(abs(imag(Node3_Radio2_RxData_wDCO(nsym_header*nsamp:end)))>sat_thresh) + ...
        sum(abs(real(Node3_Radio3_RxData_wDCO(nsym_header*nsamp:end)))>sat_thresh) + ...
        sum(abs(imag(Node3_Radio3_RxData_wDCO(nsym_header*nsamp:end)))>sat_thresh);
