...
function [s_updated N_visited_nodes] = p_dfs_det (y_received, H_Channel_Matrix, modulation_order, s_i, M_T, expa_coeff)
...
for i_step_sim=M_T:-1:(M_T - expa_coeff + 1)    
    %% PED computations:
...
for i_step_sim=M_T:-1:(M_T - expa_coeff + 1)
    ...
    [min_value, min_index]=min(ABS_SQUARE_of_e_i);
    T_i=T_i+min_value;
    N_visited_nodes (i_step_sim)= N_visited_nodes(i_step_sim) +1;
    
    if(T_i>C_i)
        finish=1;
        N_visited_nodes = ones(1,M_T);
        break;
    end
    min_vector(i_step_sim)=min_index-1;
    ...
    for mod_index=0:modulation_order-1
        if (mod_index~=min_index-1 && T_so_far(mod_index+1)<=C_i && i_step_sim~=(M_T - expa_coeff + 1))
...
%updating the estimated values at the end of the tree
if (i_step_sim==(M_T - expa_coeff + 1) && T_i<C_i)
    s_updated=min_vector;
    radius_updated=T_i;
 end
...
calculate_distance(RAM_Data(RAM_Counter,:), M_T, modulation_order, expa_coeff);

%save tmp s_updated N_visited_nodes
...
function calculate_distance(saved_data, M_T, modulation_order, expa_coeff)
global RAM_Data RAM_Counter s_updated N_visited_nodes s_i radius_updated y R T_i 
...
if (T_i>=C_i || i_step_sim==(M_T - expa_coeff + 1))
...
if (T_i<C_i || i_step_sim~=(M_T - expa_coeff + 1))
    ...
    calculate_distance(calculated_s, M_T, modulation_order, expa_coeff);
end
