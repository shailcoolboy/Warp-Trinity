function [sync_error, zf, sd] = r_partial_post(input, config, expa_coeff)
		zf = {};
		sd = {};
...
%         warning off;
 ...
 %       y_received_su = (transpose([payload_rx1(:,1), payload_rx2(:,1)]));
        
        for vec_len_cntr = 1 : vec_length(1)
            ...
            [recv_payload_SD(:,vec_len_cntr) N_visited_nodes]= p_dfs_det(y_received, hest, M, s_i, M_T, expa_coeff);
            %     vec_len_cntr
        end

%         sd{1} = recv_payload_SD(1,:);
%         sd{2} = recv_payload_SD(2,:);
        ...
        % if (num_errs.ZF>0 || num_errs.SD>0)
            % disp('BS');
        % end
