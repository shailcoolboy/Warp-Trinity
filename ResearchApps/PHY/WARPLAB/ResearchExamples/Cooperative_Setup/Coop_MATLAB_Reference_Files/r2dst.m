function [output, sat_flag, agc_read] = r2dst(input,config,rrr_cnt)
...
% Node2_TargetdBmAGC = -9;     % AGC's target dBm
% Node2_NoiseEstdBmAGC = -95;  % AGC's Noise Estimate in dBm
...
ytx1_filt = ytx1_filt./max(max([abs(real(ytx1_filt)),abs(imag(ytx1_filt))]))/sqrt(M_T);
ytx2_filt = ytx2_filt./max(max([abs(real(ytx2_filt)),abs(imag(ytx2_filt))]))/sqrt(M_T);
...
sat_thresh = 0.6;
sat_flag = sum(abs(real(Node2_Radio2_RxData_wDCO(nsym_header*nsamp:end)))>sat_thresh) + ...
...
agc_read.rd = [GainsRF_Init,GainsBB_Init];
